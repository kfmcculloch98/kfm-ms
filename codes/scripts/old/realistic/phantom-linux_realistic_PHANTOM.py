...
mf6_exe = os.path.join(notebook_dir, MODFLOW_PATH, "mf6")
...
def setup_sim(name, wel_data, ws):
    """
    create a MODFLOW 6 simulation with a 1-year weekly timeline and all-sides RIV boundaries.
    ...
    # RIV boundaries along the perimeter of the grid
    # stage set to match the initial head gradient and a small conductance to allow flow
    # river bottom is set slightly below the lowest initial head to ensure it remains active
    riv_cells = set()
    for c in range(ncol):
        riv_cells.add((0, 0, c))         
        riv_cells.add((0, nrow - 1, c))  
    for r in range(nrow):
        riv_cells.add((0, r, 0))         
        riv_cells.add((0, r, ncol - 1))  

    riv_data = []
    for k, r, c in riv_cells:
        stage = strt_gradient[c]
        rbot = stage - 1.0  
        cond = 0.05       
...
if __name__ == "__main__":
    ...
    # ==========================================
    # OPTIMIZATION MATRIX ASSEMBLY (FIXED)
    # ==========================================
        # =================================================================
    # OPTIMIZATION MATRIX ASSEMBLY (INDEX-MATCHED CONVOLUTION)
    # =================================================================
    ...
    for p_obs in range(nper):
        for i_cp in range(n_cp):
            ...
            for p_pump in range(nper):
                # Causality check: A well cannot cause drawdown BEFORE it starts pumping!
                if p_obs >= p_pump:
