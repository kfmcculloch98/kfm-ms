...
def setup_sim(name, wel_data, ws):
    ...
    flopy.mf6.ModflowGwfnpf(gwf, k=0.05, icelltype=0)
    flopy.mf6.ModflowGwfsto(gwf, ss=5e-6, transient=True)

    # set initial heads
    flopy.mf6.ModflowGwfic(gwf, strt=2377.0)
...
def run_phantom_well(task_info):
    ...
    wel_spd = {p: [((0, wr, wc), -1)] for p in range(nper)}
    ...
    for p in range(nper):
        for i, (r_cp, c_cp) in enumerate(cp_cells):
            local_R[p, i] = 2377.0 - h_data[p, 0, r_cp, c_cp]
...
if __name__ == "__main__":
        ...
        real_wells_data.append(
            {
                "r": r,
                "c": c,
                # assign random pumping rates within the specified bounds
                "Q": np.random.uniform(0.5, 1.5, nper),
            }
        )
    ...
    b_target = np.array(
        [2377.0 - hds_t[p, 0, r, c] for p in range(nper) for r, c in cp_cells]
    )
