...
def setup_sim(name, wel_data, ws):
    """
    create a MODFLOW 6 simulation with custom boundary conditions:
    - Top/Bottom: No-Flow borders (via IDOMAIN)
    - Left Border: Constant Head (CHD)
    - Right Border: Constant Head (CHD)
    ...
    # left border: Constant Head (CHD) package
    # skip the top and bottom corners because they are no-flow (range from 1 to nrow-1)
    chd_left_data = []
    for r in range(1, nrow - 1):
        # ((layer, row, col), head_value)
        chd_left_data.append(((0, r, 0), 10.0))
    
    chd_left_period_data = {per: chd_left_data for per in range(nper)}
    flopy.mf6.ModflowGwfchd(gwf, pname="chd_left", stress_period_data=chd_left_period_data)

    # right border: Constant Head (CHD) package
    # skip the top and bottom corners because they are no-flow (range from 1 to nrow-1)
    chd_right_data = []
    for r in range(1, nrow - 1):
        # ((layer, row, col), head_value)
        chd_right_data.append(((0, r, ncol - 1), 5.0))
        
    chd_right_period_data = {per: chd_right_data for per in range(nper)}
    flopy.mf6.ModflowGwfchd(gwf, pname="chd_right", stress_period_data=chd_right_period_data)
...
def run_phantom_well(task_info):
    ...
    # recall initial heads gradient from 10.0 to 5.0
    strt_gradient = np.linspace(10.0, 5.0, ncol)
    ...
    for p in range(nper):
        for i, (r_cp, c_cp) in enumerate(cp_cells):
            local_R[p, i] = strt_gradient[c_cp] - h_data[p, 0, r_cp, c_cp]
...
if __name__ == "__main__":
    ...
    # place a Phantom well in every odd-numbered perimeter cell
    phantom_locs = cp_cells[1::2] 
    n_phantom = len(phantom_locs)
    print(f"Set up {n_phantom} PHANTOM wells along odd-numbered perimeter cells...")
    ...
    print("Assembling the global surrogate matrix (G)...")
    ...
    try:
        ...
        script_dir = Path(os.getcwd()) # fallback
