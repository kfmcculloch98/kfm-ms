...
def run_parallel_inversion(level):
    ...
    pst.control_data.noptmax = 0  # Number of optimization iterations for the master run
    
    # CLEAN METHOD FOR V2: Safely clear out autogen flags from the options dictionary
    pst.pestpp_options.pop("ies_autogen_par_ensem", None)
    pst.pestpp_options.pop("ies_autogen_obs_ensem", None)
    pst.pestpp_options.pop("ies_par_en_std_dev", None) # Handled by our custom matrix draw instead
    ...
    # Loop through each parameter to generate log-uniform draws within specified bounds
    for idx, (p_name, row) in enumerate(par_df.iterrows()):
        # Pull low and high boundaries for this specific parameter channel
        lb = max(row["parlbnd"], 1e-4) # Prevent log(0) constraints
        ub = row["parubnd"] 
        
        # Draw uniformly in log-space, then transform back to linear pumping values
    ...
    # Set the model command to invoke the forward run script using the virtual environment's Python interpreter
    pst.model_command = "python forward_run.py"
    
    # SECURE V2 WRITE: Explicitly clean control data overrides before rendering the V2 file
    # This prevents pyemu from creating phantom duplicated sections in the text header
