...
def build_modular_pst(level, root_name):
    ...
    # Relative target weighting
    ERROR_PERCENTAGE = 0.20  # 20% error standard deviation metric
    print(f"[+] Calculating dynamic relative observation weights (w = 1 / ({ERROR_PERCENTAGE} * target))...")
    
    floor_threshold = 0.01   # Keeps very close-to-zero nodes from dividing by 0
    
    # Isolate targets and calculate their standard deviations
    targets = pst.observation_data["obsval"].copy()
    
    # Enforce a floor on the denominator to prevent division-by-zero or massive weights at absolute 0
    clamped_targets = np.maximum(targets, floor_threshold)
    
    # Calculate and assign the inverse standard deviation weights
    pst.observation_data.loc[:, "weight"] = 1.0 / (ERROR_PERCENTAGE * clamped_targets)
    
    # Mute structural noise elements completely below your tracking limit if desired
    dead_zone_mask = targets < 0.01
    pst.observation_data.loc[dead_zone_mask, "weight"] = 0.0
    
    print(f"    Dynamic scaling applied. Max weight: {pst.observation_data['weight'].max():.2f}")
    print(f"    Min weight (active): {pst.observation_data.loc[~dead_zone_mask, 'weight'].min():.2f}")
    ...
    pst.parameter_data.loc[:, "parval1"] = 0.15  # multiplier for pumping rates  
    pst.parameter_data.loc[:, "parlbnd"] = 0.01  # Lower bound multiplier floor
    pst.parameter_data.loc[:, "parubnd"] = 15.0  # Upper bound multiplier cap
    ...
    if level == "1":
        ...
        pst.parameter_data.loc[active_mask, "parval1"] = 0.15 
        ...
        for _, row in unique_locs.iterrows():
            ...
            pst.parameter_data.loc[mask, "parval1"] = 0.15

    elif level == "3":
        print("[+] You selected Level 3 (Blind)")
        pst.parameter_data.loc[:, "partrans"] = "none"
        pst.parameter_data.loc[:, "parval1"] = 0.15
